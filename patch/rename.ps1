function Invoke-Name-Patch{
    ...
    Get-ChildItem -Path $rootPath -Recurse -File | Where-Object { $extensions -contains $_.Extension } | ForEach-Object {
        ...
        $data = $data.Replace("blockball", "mcbasketball").replace("BlockBall", "MCBasketball").replace("Soccer", "Basketball").replace("soccer", "basketball").replace("/football", "")
        Set-Content -Value $data -Path $filePath

        $relativePath = $filePath.Substring($rootPath.Length).TrimStart("\\")
        $newRelativePath = $relativePath.Replace("blockball", "mcbasketball").replace("BlockBall", "MCBasketball").replace("Soccer", "Basketball").replace("soccer", "basketball")
